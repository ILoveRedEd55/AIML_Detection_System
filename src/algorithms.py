...
from net.visual import write_on_image, visualise, activity_dict, visualise_tracking
from net.processor import Processor
from helpers import pop_and_add, last_ip, dist, move_figure, get_hist
from default_params import *
from net.inv_pendulum import *
import re
import pandas as pd
from scipy.signal import savgol_filter, lfilter
from net.model import LSTMModel
import torch
import math
...
def get_source(args):
    tagged_df = None
    cam = cv2.VideoCapture(0, cv2.CAP_DSHOW) #1 assigned index for camera
    img = cam.read()[1]
...
def alg2_sequential(queues, argss, consecutive_frames, event):
    ...
    model.load_state_dict(torch.load('./src/net/lstm_weights.sav',map_location=argss[0].device))
    model.eval()
    ...
    [cv2.namedWindow(window_name, flags=cv2.WINDOW_GUI_NORMAL) for window_name in window_names]
    [cv2.setWindowProperty(window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN) for window_name in window_names]
    [cv2.resizeWindow(window_name, 800, 600) for window_name in window_names]
    [cv2.moveWindow(window_name, 370, 75) for window_name in window_names]
    [cv2.setWindowProperty(window_name, cv2.WND_PROP_TOPMOST, 1) for window_name in window_names]
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if cv2.waitKey(1) == 113 or any(cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE) < 1 for window_name in window_names):
                if not event.is_set():
                    event.set()
            ...
            if argss[0].num_cams == 1:
                ...
                dict_frames[0]["tagged_df"]["text"] += f""
